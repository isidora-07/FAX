...
loginSucces = function(data, textStatus, jqXHR){
  ...
  $.getRole(_url + "login/userRole?username=" + username + "&pass=" + password);
}

switchScreen = function(data, textStatus, jqXHR){
  var moj_json = JSON.stringify(data);
  var response = JSON.parse(moj_json);
  sessionStorage.setItem("role",response[0].user_role);
...
$.FillTableBends = function(){
  ...
  var innerString = "";
  innerString="<tr onclick='getArtistsByBendClick(-1)'><th>Naziv benda </th><th>Žanr</th>";
...
$.FillTableGenres = function(){
  ...
  var unos = document.getElementById("unos_div");
  if(sessionStorage.getItem("role") != "admin"){
